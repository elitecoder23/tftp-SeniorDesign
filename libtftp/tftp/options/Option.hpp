	...
	namespace Options
	{
		...
		class Option
		{
			public:
				 ...
				 * RRQ/ WRQ packets. The Server is allowed to modify the option value to
				 * a value, which is accepted by the client.
				 *
				...
				virtual OptionPointer negotiateServer(
					const string &optionValue) const = 0;

				/**
				 * @brief Option negotiation on client side.
				 *
				 * This function is called, when the TFTP client receives options within
				 * a OACK packet. The client will or will not accept the receipt options
				 *
				 * @param[in] optionValue
				 *   The input option
				 *
				 * @return Option negotiation result
				 * @retval OptionPointer(0)
				 *   Option negotiation failed on client side. Error packet will be sent.
				 **/
